...
argin.addParamValue('replicates', 1, @(x) isnumeric(x) && x>0);
...
for rep = 1:replicates
  ...
  for iter = 1:maxiter
    ...
    if i == dx
      ...
      mat = cal_malt(betax.U,dx,i,cumkrx);
      matlt = kron(mat', eye(px(i)));
%       matlt = kron(khatrirao([betax.U(dx:-1:i+1),cumkrx])', eye(px(i)));
    ...
    if j == dy
      ...
      mat = cal_malt(betay.U,dy,j,cumkry);
      matrt = kron(mat, eye(py(j)));
    ...
    Cxx = Cxx + 1e-3 * speye(size(Cxx, 1));
    Cyy = Cyy + 1e-3 * speye(size(Cyy, 1));
...
% 
    ...
    B = ([Cxx zeros(size(Cxy)); zeros(fliplr(size(Cxy))), Cyy]);
    
    objvalold = objval;
    [evec, objval] = eigs(double(A), double(B), 1, 'lm', opts);
...
%     Cxx_inv = Cxx^(-0.5); Cyy_inv = Cyy^(-0.5);
%     Cxx_inv = real(Cxx_inv); Cyy_inv = real(Cyy_inv);
%     T = Cxx_inv*Cxy*Cyy_inv;
%     [U,D,V] = svd(T);
%     u_x = Cxx_inv*U(:,1);
%     u_y = Cyy_inv*V(:,1);
%     objvalold = 0;objval = 0;

%     if(dx == 100)
%         betax{i} = Cxx_inv * U(:,1:rx);
%     else
%         betax{i} = reshape(u_x,[px(i), rx]);
%     end
% 
%     if(dy == 100)
%          betay{j} = Cyy_inv * V(:,1:ry);
%     else
%          betay{j} = reshape(u_y,[py(j), ry]);
%     end
...
%     betax{i}(abs(betax{i}) < xisort(min(xl0maxprop, px(i) * rx))) = 0;
%     yjsort = sort(abs(vec(betay{j})), 'descend');
% %     betay{j}(abs(betay{j}) < yjsort(min(floor(yl0maxprop*py(j)), py(j) * ry))) = 0;
%     betay{j}(abs(betay{j}) < yjsort(min(yl0maxprop, py(j) * ry))) = 0;

    betax{i}(abs(betax{i}) < lambda) = 0;
    betay{j}(abs(betay{j}) < lambda) = 0;
...
function malt = cal_malt(U,dx,i,cumkr)

malt = U{dx};
for i = dx-1:-1:i+1
    malt = khatrirao(malt, U{i});
end

malt = khatrirao(malt, cumkr);
