...
if Nlayers==0
  ...
  for j=1:Nlayers-1
    ...
    if strcmp(layer.type,'conv')
      ...
      %       OUT.respfull_prepool=gpuArray.zeros(N,layer.sizeout_prepool1,layer.sizeout_prepool2,layer.numoutputmaps);
      OUT.respidx=zeros(N,layer.sizeout_prepool1,layer.sizeout_prepool2,layer.numoutputmaps);
      %       OUT.respidx=gpuArray.zeros(N,layer.sizeout_prepool1,layer.sizeout_prepool2,layer.numoutputmaps);
      OUT.respfull=zeros(N,layer.sizeout1,layer.sizeout2,layer.numoutputmaps);
      %       OUT.respfull=gpuArray.zeros(N,layer.sizeout1,layer.sizeout2,layer.numoutputmaps);
      for filteridx=1:layer.numoutputmaps
        ...
        % In case of NaN, set the value to be very small for maxpooling.
        resp(isnan(resp))=-1984;
  ...
  switch lower(layer.type)
    ...
    case 'softmax',
      tttt=median(T,2);  T=bsxfun(@minus, T, tttt);
      T=exp(T); s=sum(T,2); % T=diag(sparse(1./s))*T;
      T=bsxfun(@times,1./s,T); T(T<1e-16)=1e-16;
      E=E - sum(sum(Y.*log(T))); if isnan(E) max(s), min(s), max(T(:)),min(T(:)), error('is nan'); end;
    otherwise,
      error('Invalid layer type: %s\n',layer.type);
  end
  WW{j}=W;
  ...
  for j=Nlayers-1:-1:1
    ...
    if strcmp(layer.type,'conv')
      ...
      % Nan can appear due to padding of variable length inputs.
      lowerlayeroutput(isnan(lowerlayeroutput))=0;
      ...
      % de=gpuArray.zeros(1, layer.filternumrows*layer.filternumcols*layer.numinputmaps,...
