function [Ux, Uy, Mx, My] = STCCA_Wang_joint(X,Y, lambda_u, lambda_v, dim)
%STCCA 此处显示有关此函数的摘要
%   此处显示详细说明
...
max_iters = 10; epsilon_corr = 0.005;
...
% end
Sxx = Sxx/N + 0.001 * eye(Fx);

% if(Fy > N)
%     Syy = Syy + 0.001 * eye(Fy); % regularized CCA
% end
Syy = Syy/N + 0.001 * eye(Fy);
...
for j = 1:r_dim
    ...
    for iter = 1:100
        C_u = C'*u_old;
        v_new = sign(C_u) .* max(0, abs(C_u) - lambda_v*norm(u_old,1)/2);
        if(norm(v_new) == 0)
            v_new = v_new;
        else
        ...
        C_v = C * v_new;
        u_new = sign(C_v) .* max(0, abs(C_v) - lambda_u*norm(v_new,1)/2);
        if(norm(u_new) == 0)
            u_new = u_new;
        else
