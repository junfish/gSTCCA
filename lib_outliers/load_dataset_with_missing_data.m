function [X,Y,label] = load_dataset_with_missing_data(dataset_name, data_normalization, ratio)
...
if(strcmp(dataset_name, 'Mnist_012_processed_new') == 1)
    ...
    X = generate_missing_by_ratio(X, ratio);
    Y = generate_missing_by_ratio(Y, ratio);
end
